...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  describe '一覧表示機能' do
   ...
   let!(:second_task) { FactoryBot.create(:second_task, title: 'second_task',  content: '2番目のタスク説明', status: :not_started, created_at:1.hour.ago)}
   let!(:third_task) { FactoryBot.create(:third_task, title: 'third_task', content: '3番目のタスク説明', status: :not_started, created_at:1.minute.ago)}
  #new_task追記
  ...
  describe 'ソート機能' do
    ...
    context '「優先度」というリンクをクリックした場合' do
      it "優先度の高い順に並び替えられたタスク一覧が表示される" do
        ...
        visit tasks_path
        click_link '優先度'
        task_titles  = all('.task_priority').map(&:text)
  ...
  describe '検索機能' do
    context 'タイトルであいまい検索をした場合' do
      ...
      # toとnot_toのマッチャを使って表示されるものとされないものの両方を確認する
        visit tasks_path
        select 'search[title]', from: 'first'
        click_button '検索'
